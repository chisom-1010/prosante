...
const columns: ColumnDef<DoctorRow>[] = [
  ...
  },
  {
    ...
    header: "Email",
    cell: ({ row }) => (
      <div className="text-muted-foreground">{row.original.doctor_email}</div>
    ),
  },
  {
    accessorKey: "service",
    header: "Service",
    cell: ({ row }) => <div>{row.original.service}</div>,
  },
  {
...
export default function DoctorsList() {
  ...
  React.useEffect(() => {
    const fetchDoctors = async () => {
      ...
      const { data, error } = await supabase.rpc("get_doctors_dashboard");

      if (error) {
        console.error("Failed to fetch doctors dashboard:", error);
        setLoading(false);
        return;
      }
      ...
      if (!parsed.success) {
        console.error("Invalid doctors dashboard payload:", parsed.error);
        setLoading(false);
        return;
      }
      ...
      <div className="flex items-center justify-between px-4 lg:px-6">
        <div className="flex items-center gap-2">
          ...
          <Input
            ...
            value={
              (table.getColumn("doctor_name")?.getFilterValue() as string) ?? ""
            }
            onChange={(event) =>
